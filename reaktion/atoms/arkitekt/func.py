...
class FuncNoParallelArkitektAtom(Atom):
    ...
    def __init__(self, diagramNode: DiagramNode, outQueue: asyncio.Queue, reservation: Reservation = None, constants: Dict = {}, maxsize:int = 0, **kwargs) -> None:
        ...
        try:
            while True:
                ...
                if isinstance(event, YieldEvent) or isinstance(event, ReturnEvent):
                    await self.log(f"Node Upstream yielded or returned {event.returns}. Assigning as Function")

                    try:
                        result = await asyncio.wait_for(self.reservation.assign_async(*event.returns,**self.constants, bypass_shrink=True, bypass_expand=True), timeout=self.timeout)
