        ...
        try:
            while True:
                ...
                if isinstance(event, YieldEvent) or isinstance(event, ReturnEvent):
                    await self.log(f"Node Upstream yielded or returned {event.returns}. Assigning to Generator")

                    try:
                        async for result in self.reservation.stream_async(*event.returns,**self.constants, bypass_shrink=True, bypass_expand=True):
