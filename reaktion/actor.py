...
from reaktion.events import CancelEvent, DoneEvent, ErrorEvent, EventType, ReturnEvent, TransitionEvent, YieldEvent
...
class ReactiveGraphActor(Actor):
    ...
    async def on_reservation_transition(self, diagramID: str, reservation: Reservation, state: ReserveState):
        await self.provide_log(f"Event: {json.dumps(TransitionEvent(diagram_id=diagramID, reservation=reservation.reference, state=state, message='Caused by a Transition').dict())}")
    ...
    async def on_provide(self, provision: BouncedProvideMessage):
        ...
        self.diagramNodeIDReservationMap = await self.engine.generateReservationsMap(provision.meta.reference, provision.meta.context, self.on_reservation_transition)
    ...
    async def on_unprovide(self, provision: BouncedProvideMessage):
        ending_futures = [ res.end() for id,res in self.diagramNodeIDReservationMap.items()]
    ...
    async def assign_log(self, assign: BouncedAssignMessage, message: str, level: LogLevel = LogLevel.INFO):
    ...
    async def assign_generator(self, args, kwargs, assign):
        assert len(self.engine.argNode.data.args) == len(args), "Received different arguments then our Engines ArgNode requires"
        ...
        nodeIDAtomsMap = await self.engine.generateAtoms(outQueue, nodeIdConstantsMap, self.diagramNodeIDReservationMap, log=log)
        ...
        try:
            while True:
                ...
                handle_nodes = self.engine.connectedNodesWithHandle(event.diagram_id, event.handle)
                ...
                for handle, diagramNode in handle_nodes:
                    ...
                    if isinstance(event, YieldEvent) or isinstance(event, ReturnEvent):
                        if isinstance(diagramNode, diagram.ReturnNode):
                            await log(f"Event: {json.dumps(YieldEvent(diagram_id=diagramNode.id, returns=event.returns, handle='returns').dict())}")
                            yield event.returns
                        else:
                            await nodeIDAtomsMap[diagramNode.id].assign_handle(handle, event)

                    if isinstance(event, DoneEvent):
                        if isinstance(diagramNode, diagram.ReturnNode):
                            await log(f"Event: {json.dumps(DoneEvent(diagram_id=diagramNode.id, handle='returns').dict())}")
                            return
                        else:
                            await nodeIDAtomsMap[diagramNode.id].assign_handle(handle, event)
                    ...
                    if isinstance(event, ErrorEvent):
                        if isinstance(diagramNode, diagram.ReturnNode):
                            await log(f"Event: {json.dumps(ErrorEvent(diagram_id=diagramNode.id, exception=event.exception, message=event.message, throwing=event.throwing, handle='returns').dict())}")
            ...
            await log(f"Event: {json.dumps(CancelEvent(diagram_id=self.engine.argNode.id,handle='args').dict())}")
            await log(f"Event: {json.dumps(CancelEvent(diagram_id=self.engine.kwargNode.id,handle='args').dict())}")
            await log(f"Event: {json.dumps(CancelEvent(diagram_id=self.engine.returnNode.id,handle='args').dict())}")
    ...
    async def assign_function(self, *args, **kwargs):
        try:
            logger.info("Assigning as Function")
            return [5]
        except asyncio.CancelledError as e:
            logger.info("Assigning as Function")
    ...
    async def on_assign(self, message: BouncedForwardedAssignMessage):
        try:
            ...
            args, kwargs = message.data.args, message.data.kwargs # We are not expanding the inputs as they are just passed on
            ...
            if self.node.type == NodeType.FUNCTION:
                ...
                await self.transport.forward(AssignReturnMessage(data={
                    "returns": returns or [] # We are not shrinking the outputs as they are already shrinked
                }, meta = {
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.diagramNodeReservation: Dict[diagram.ArkitektNode.id, Reservation]= {} # A map of