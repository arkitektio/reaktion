...
from reaktion.atoms import FuncNoParallelArkitektAtom, GenNoParallelArkitektAtom, WithLatestFromAtom
...
class Engine:
    ...
    def __init__(self, di: diagram.Diagram) -> None:
        ...
        self.noneIONodes = [ node for node in di.elements if isinstance(node, diagram.Node) and not isinstance(node, diagram.IONode)]
        self.arkitektNodes = [node for node in  self.nodes if isinstance(node, diagram.ArkitektNode)]
        self.reactiveNodes = [node for node in  self.nodes if isinstance(node, diagram.ReactiveNode)]
    ...
    def getInitialNodes(self):
        ...
        non_generic_nodes_that_are_not_targets = [node for node in self.arkitektNodes if node.id not in non_kwargs_edge_targets]
    ...
    def generateConstantsMap(self, kwargs):
        ...
        for kwarg, value  in kwargs.items():
            for kwarg_handle, node in self.connectedNodesWithHandle(self.kwargNode.id, f"kwarg_{kwarg}"):
                kwarg = kwarg_handle[len("kwarg_"):] # Trim awai kwarg_ so that key is stillt here
                nodeIDConstantsMap.setdefault(node.id, {})[kwarg] = value

        return nodeIDConstantsMap


    async def generateReservationsMap(self, causing_provision: str, context: Context, transition_hook: Coroutine):
        ...
        return { node_id: reservation for node_id, reservation in zip(diagramIDs, reservations)}
    ...
    async def generateAtoms(self, outQueue, constants= {}, reservations = {} , **kwargs):
        ...
        for diagramNode in self.noneIONodes:
            diagramIDAtomMap[diagramNode.id] = registry.get_atom(diagramNode._type)(diagramNode, outQueue, reservation=reservations.get(diagramNode.id,None), constant=constants.get(diagramNode.id, {}), **kwargs)
